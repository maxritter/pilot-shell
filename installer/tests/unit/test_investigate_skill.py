...
def test_claude_artifact_is_manual_read_only_and_complete() -> None:
    ...
    assert "steps/01-frame.md" in content
    assert "steps/02-trace.md" in content
    assert "steps/03-challenge.md" in content
    assert "steps/04-report.md" in content
...
def test_manifest_exposes_discriminating_investigation_evals() -> None:
    ...
    assert all("read its required phase resources" in case["prompt"] for case in payload["evals"])
...
def test_local_installer_writes_runtime_codex_artifacts(tmp_path: Path) -> None:
    ...
    assert (installed / "steps" / "01-frame.md").is_file()
