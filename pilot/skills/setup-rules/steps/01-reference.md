...
| Tracked project skills | `.agents/skills/<name>/` | Canonical source for both agents |
| Tracked Claude Code skill mirror | `.claude/skills/<name>/` | Generated by `scripts/sync-agent-assets.mjs`; never edit directly |
...
- Author project skills only in `.agents/skills/`; `.claude/skills/` is a generated mirror.
- Let Pilot's shared hook synchronize on SessionStart, supported edits from either agent, and Stop as a Code Mode backstop. A tracked edit aimed at `.claude/skills/` must be redirected or blocked with the canonical `.agents/skills/` path; content never flows from the mirror back into the source.
