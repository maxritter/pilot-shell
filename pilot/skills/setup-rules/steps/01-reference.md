...
| Automatic synchronization | Pilot's shared hook | Runs on SessionStart and after edits made through either Claude Code or Codex |
...
- Let Pilot's shared hook synchronize after edits from either agent. A tracked edit aimed at `.claude/skills/` must be redirected or blocked with the canonical `.agents/skills/` path; content never flows from the mirror back into the source.
