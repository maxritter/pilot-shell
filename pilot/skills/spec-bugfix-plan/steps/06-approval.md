## Step 6: Approval and Implementation Handoff
...
<!-- CC-ONLY -->
### 6.0 Prepared native-planning path
...
```bash
~/.pilot/bin/pilot notify plan_approval "Bugfix plan Ready for Review" "<plan_name> — annotate in Console or approve here" --plan-path "<plan_path>" 2>/dev/null || true
```
...
For an asynchronous or absent structured tool, read `$HOME/.pilot/agents/agent-gate-protocol.md`. Supply `GATE_NAME` = `Bugfix plan approval`, the choices above, and `SENTINEL_PATH` = `spec-approval-pending`. Create the sentinel before yielding:
...
- **Approval:** set `Approved: Yes` and continue immediately to 6.3.
- **Feedback:** process Step 5, update the plan, then ask for approval of the revised result. Do not record a guessed answer.
