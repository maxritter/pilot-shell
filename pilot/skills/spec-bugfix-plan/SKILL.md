...
3. **⛔ MANDATORY — NEVER skip or defer.** Wait for the background task completion notification.

4. **When the notification arrives**, read the background bash output. **Filter out `[codex]` prefixed log lines** — use `ctx_execute_file` to extract only non-`[codex]` lines. Look for the `# Codex Adversarial Review` section. Extract `Verdict:` and `Findings:` lines. Map severities: `[high]` / `[critical]` → must_fix, `[medium]` / `[low]` → should_fix. Fix all must_fix/should_fix in the plan.

5. **If the background bash timed out or failed** (exit code non-zero): Re-launch synchronously and wait. Only skip if the second attempt also fails.
