...
⛔ **"And no more" counts turn ends, not just questions.** A fifth interaction point does not have to be an `AskUserQuestion` — ending your turn and waiting for the user to say `continue` is the same interruption wearing a finish message, and it is worse, because the user gets no prompt telling them the run is waiting on them. The one that shipped: Manual Model Switching paused after plan approval so the user could type `/model`, which turned approval — the moment the user just said go — into a dead stop with zero tasks done. **Approval hands off directly to implementation in every Model Switching mode.** The stop guard enforces this: no sentinel grants a stop for an approved `PENDING` plan, so a turn end there is blocked and pushed back into implementation. If a model switch matters to a user mid-run, they interrupt and run `/model` — that is one keystroke when they want it, against a round-trip on every run when they don't.
