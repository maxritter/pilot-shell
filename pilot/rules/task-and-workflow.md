...
**Which model is serving the planning leg is also the hooks' job, not yours.** They read the statusline's resolved model every turn and inject a `PLANNING-LEG MODEL CHECK` warning if it is not Opus. Do NOT go inspecting `~/.pilot/sessions/<id>/context-pct.json` or reasoning about token counts to work it out — no warning means no observed mismatch. When the warning does arrive, do exactly what it says: tell the user in one short paragraph which model planning is on, then keep planning on it. It is **not** a new approval gate — `/spec` has four user interaction points and this is not one of them, so never stop to ask "compact / accept Sonnet / switch to Manual".
<!-- /CC-ONLY -->
