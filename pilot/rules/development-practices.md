...
- **⛔ Never write a repo-local git identity.** `git config user.email <value>` / `user.name <value>` is hook-blocked: repo config beats `~/.gitconfig`, so a throwaway identity silently re-authors every later commit, and the author is part of the commit hash — only a history rewrite undoes it. Reads and explicit `--global` / `--system` writes are fine. Need a scratch identity for a temp repo? Use `git -c user.email=… commit` per command, or set it inside the temp repo only.
