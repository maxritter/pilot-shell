...
remove_codex_files() {
	...
	if [ -f "$hooks_file" ] && command -v python3 >/dev/null 2>&1; then
		PILOT_CODEX_HOOKS="$hooks_file" PILOT_CODEX_HOOKS_BASELINE="$hooks_baseline" python3 -c '
import json, os, sys

hooks_path = os.environ["PILOT_CODEX_HOOKS"]
baseline_path = os.environ["PILOT_CODEX_HOOKS_BASELINE"]
try:
...
for event in list(hooks.keys()):
    ...
    for entry in hooks[event]:
        ...
        if baseline_sigs:
            ...
            commands = [
                str(h.get("command", "")) for h in entry.get("hooks", []) if isinstance(h, dict)
            ]
            is_pilot = any(
...
' 2>&1
	...
	for skill in "${CODEX_PILOT_SKILLS[@]}"; do
		...
		if [ -f "$resources_manifest" ] && command -v python3 >/dev/null 2>&1; then
			PILOT_CODEX_SKILL_DIR="$skill_dir" python3 -c '
import json, os
